...
class FailureInjector:
    ...
    @classmethod
    def from_file(cls, path: Union[str, Path]) -> "FailureInjector":
    ...
    def evaluate_failure(self, command_str: str) -> Tuple[FailureAction, Any]:
        ...
        # 1. Check Random Disconnect
        if (
            self.config.disconnect_probability > 0
            and random.random() < self.config.disconnect_probability
        ):
            logger.info("Failure injected: Random disconnect triggered for command '%s'", cmd_clean)
        ...
        # 5. Check Response Delay
        if self.config.maximum_delay_ms > 0 and self.config.maximum_delay_ms >= self.config.minimum_delay_ms:
